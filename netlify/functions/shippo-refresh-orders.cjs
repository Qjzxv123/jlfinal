// netlify/functions/shippo-refresh-orders.cjs
...
exports.handler = async (event) => {
  ...
  console.log('[Shippo Refresh] Function invoked');

  const SHIPPO_API_KEY = process.env.SHIPPO_API_KEY;
  if (!SHIPPO_API_KEY) {
    console.log('[Shippo Refresh] Missing Shippo API token');
  ...
  try {
    ...
    if (historyError) {
      console.warn('[Shippo Refresh] Error fetching Order History:', historyError.message);
    }
    ...
    if (mappingError) {
      console.warn('[Shippo Refresh] Error fetching SKU Mappings:', mappingError.message);
    }
    ...
    console.log(`[Shippo Refresh] Processed ${processedCount} orders, skipped ${skippedCount} (already in history)`);
  } catch (e) {
